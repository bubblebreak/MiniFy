...
var durationValueInput;
var updateDurationButton;
var frameRateInput;
var updateFrames;
...
var representationSetting;

var saturationCanvas;
var ctx;
var canvasWidth = 640;
...
export function setupDanceCollection() {
    ...
    canvas1.setAttribute("hidden", true);
    ...
    let inputDanceWrapper = document.createElement("div");
    inputDanceWrapper.style.display = "none";

        let inputDanceCollapseButton = document.createElement("button");
        inputDanceCollapseButton.setAttribute("id", "inputDanceCollapseButton");
        inputDanceCollapseButton.textContent = "Show/hide settings";
        inputDanceCollapseButton.addEventListener("click", () => {
            if(inputDanceWrapper.style.display === "none"){
                inputDanceWrapper.style.display = "block";
            } else{
                inputDanceWrapper.style.display = "none";
            }
        });
        ...
        let inputDanceDuration = document.createElement("div");
            ...
            danceDurationInput.value = duration/1000;
            danceDurationInput.addEventListener("change", updateDuration);
            danceDurationInput.addEventListener("keypress", e =>{if(e.key === "Enter"){updateDuration()}});
            ...
            danceDurationUpdateButton.addEventListener("click", updateDuration);
        ...
        let inputFrameRate = document.createElement("div");
            ...
            frameRateUpdateButton.addEventListener("click", updateFrameRate);
        ...
        let inputSensitivity = document.createElement("div");
            ...
            sensitivityInput.setAttribute("max", "100");
            sensitivityInput.setAttribute("value", "50");
            ...
            sensitivityInput.addEventListener('input', () => {
                let mappedSensitivity = 100-Math.floor(((sensitivityInput.value - minDiff) / (maxDiff - minDiff)) * 100);
                sensitivityValueDisplay.textContent = `${mappedSensitivity}`;
                maxRange = sensitivityInput.value;
            });
    ...
    entryContainer.appendChild(inputDanceWrapper);
    entryContainer.appendChild(inputDanceCollapseButton);
    inputDanceWrapper.appendChild(inputDanceDuration);
    inputDanceWrapper.appendChild(inputFrameRate);
    inputDanceWrapper.appendChild(inputSensitivity);

    setDanceValues();
}

function setDanceValues(){
    /////////// Code for calculating the pixel difference
    remainingTimeDisplay = document.getElementById("timeDisplay");
    durationValueInput = document.getElementById("collectDuration");
    updateDurationButton = document.getElementById("updateDuration");
    frameRateInput = document.getElementById("frameRate");
    updateFrames = document.getElementById("updateFrames");
    ...
    diffDisplay = document.getElementById('diffValue');

    // Controls to display/hide video and noise
    representationSetting = document.getElementById("setView");

    saturationCanvas = document.getElementById('saturationCanvas');
    ctx = saturationCanvas.getContext('2d');
    canvasWidth = saturationCanvas.width;
    canvasHeight = saturationCanvas.height;

    mappedDiffDisplay = document.getElementById('mappedDiffValue');
